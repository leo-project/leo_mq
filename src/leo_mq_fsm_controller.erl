...
handle_sync_event(state, _From, StateName,
                  #state{status = Status,
                         total_num_of_targets = TotalNumOfTargets,
                         reserved_targets     = ReservedTargets,
                         pending_targets      = PendingTargets,
...
loop(CallbackFun, TargetId) ->
    receive
        {run, Id} ->
            ok = leo_mq_fsm_worker:run(Id, self(), CallbackFun),
            Id_1 = [],
        ...
        {lock, Id} ->
            ...
            loop(CallbackFun, TargetId);
        {suspend,_Id} ->
            ok = leo_mq_fsm_worker:suspend(TargetId),
            loop(CallbackFun, TargetId);
        {resume,_Id} ->
            ok = leo_mq_fsm_worker:resume(TargetId),
            loop(CallbackFun, TargetId);
        {finish, Id} ->
            _  = finish(self(), Id),
            loop(CallbackFun, TargetId);
